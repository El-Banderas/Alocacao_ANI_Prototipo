...
class MyServer(BaseHTTPRequestHandler):
    def __init__(self, tecns : list[Tech], projs : list[Proj], attribution, compabilities, *args, **kwargs):
        self.tecns = tecns
        self.projs = projs
    ...
    def get_attribution(self):
        ...
        for id_proj, tecn in enumerate(self.attribution):
            ...
            tasks_costs_relative_to_tecn[f'Task{id_proj}'] = cost_this_project
            if tecn not in attributions:
                attributions[f'Tecn{tecn}'] = []
            attributions[f'Tecn{tecn}'].append(f'Task{id_proj}')
        return {"input": {
    ...
    def do_GET(self):
        ...
        if "proj" in query_components:
            answer = self.get_proj(proj_id=int(query_components["proj"]))
        elif "tecn" in query_components:
            answer = self.get_tecn(tecn_id=int(query_components["tecn"]))
...
def server_main(input, atributtion):
    handler = partial(MyServer, input.excel_information.tecns, input.excel_information.tasks, atributtion, input.excel_information.compatibilities)
